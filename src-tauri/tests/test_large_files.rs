...
async fn empty_directory_returns_empty_vec() {
    ...
    assert!(result.is_ok());
    assert!(result.unwrap().is_empty(), "Empty directory must yield empty list");
}
...
async fn first_result_is_always_the_largest_file() {
    ...
    assert!(files[0].name.contains("e.bin"), "Name must be e.bin, got {}", files[0].name);
}
...
async fn limit_is_strictly_respected() {
    ...
    assert_eq!(files.len(), 3, "Result must contain exactly `limit` entries");
}
...
async fn limit_larger_than_total_count_returns_all_files() {
    ...
    assert_eq!(files.len(), 5, "All 5 files must be returned when limit > count");
}
...
async fn directories_are_excluded_from_results() {
    ...
    assert_eq!(files.len(), 1, "Only files, not directories, must be returned");
    assert!(!files[0].is_dir);
}
...
async fn files_in_subdirectories_are_traversed_recursively() {
    ...
    assert!(names.contains(&"d.bin"), "Nested d.bin must appear in results");
    assert!(names.contains(&"e.bin"), "Nested e.bin must appear in results");
}
...
async fn each_file_node_has_correct_is_dir_and_file_count_flags() {
    ...
    for file in &files {
        ...
        assert_eq!(file.file_count, 1, "file_count must be 1 for each file node");
    }
}
...
async fn path_field_is_an_absolute_path_pointing_to_the_file() {
    ...
    assert!(path.is_absolute(), "path field must be absolute: {}", files[0].path);
...
async fn nonexistent_path_returns_empty_or_error() {
    ...
    let result =
        get_large_files("/nonexistent/path/xyz_abc_does_not_exist".to_string(), 10).await;
